  ...
  for(i=0; i<S; i++)
    {
      ...
      if((i==2) || (i==3))	
	  {
	    ...
	    flux[i][4]=0; //bottom
	    flux[i][5]=0; //top   
	    flux[i][0]=beamfactor*flux[i][0];
	    flux[i][1]=beamfactor*flux[i][1];
	  }
#endif
      ...
      if((i==2) || (i==3))	
	{
      ...
      totalflux[i]=(flux[i][0]+flux[i][1]+flux[i][2]+flux[i][3]+flux[i][4]+flux[i][5]);
      if(rank==0)
	printf("obs vdrift SPECIE %d: %E Flux: %f, %f, %f, %f, %f, %f\n", i, vdriftx[i],flux[i][0], flux[i][1], flux[i][2], flux[i][3], flux[i][4], flux[i][5]);
      //	arger=arger;
      //arger=0;
