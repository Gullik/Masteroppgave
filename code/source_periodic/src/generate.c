...
void gen_bgnd(void)
{
  ...
  int where;
   	   ...
   	   init_primeroot(rank/(numtasks*1.0)); //Z
      ...
      for(j=0; j<npart[i]; j++) /*for each particle*/
	{
	  do //this loop is for not having particles in the object :)
		...
		{		  
	      ...
	      if(i==2)
		{
		  spec[i].part[j].y=qy*0.5*Ly+0.25*Ly;
		  spec[i].part[j].z=qz*0.5*Lz+0.25*Lz;
		}
	      if(i==3)
	      	{
		  spec[i].part[j].y=qy*0.5*Ly+0.25*Ly;
	  ...
	  /*	spec[i].part[j].x=0.5*Lx;
		...
		spec[i].part[j].z=0.5*Lz;
		spec[i].part[j].vy=1.0*vthx[0];
		spec[i].part[j].vz=0.0;
		spec[i].part[j].vx=0.5*vthx[0];
	  */
...
double rra;
 ...
 printf("before newparticles 189558 %E %E %E\n",spec[0].part[189558].x,spec[0].part[189558].y,spec[0].part[189558].z);
...
//	    v=vdriftx[i];
	  ...
	  if((i==2) || (i==3))
		{
		  spec[i].part[j].y=r2*0.5*Ly+0.25*Ly;
		  spec[i].part[j].z=r3*0.5*Lz+0.25*Lz;
		}
#endif
