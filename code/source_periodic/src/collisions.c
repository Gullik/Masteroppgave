  ...
  for(i=0; i<S; i++)
   ...
   //getchar();
     for(ii=0; ii<ii_max; ii++)
	{
     ...
     printf("nullcollisions percentage, specie %d: %E freq %E\n normtime %E coll pr dt %E\n", i, nullcoll[i], nullcollfreq[i], normtime, nullcoll[i]*npart[i]);
     printf("MEAN FREE PATH FOR %d: %E m\n", i, 1/(fs*ndensity));
      getchar();
    }
}
...
void collisions(void)
{
  ...
  double rr, rr2;
  double kener, pvel, collfreq, before, after;
  ...
  for(i=0; i<S; i++)
      ...
      for(ii=0; ii<noofcoll; ii++)
	{
	  ...
	  for(type=0; type<colltypes[i]; type++)
	    {	
	      collfreq=pvel*sigma[type]*ndensity;
	      after+=collfreq;
