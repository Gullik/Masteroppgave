...
void redistribute(double coeff)
{
  int i,ii;
  double total, redist;
  for(i=0;i<noofdusts;i++)
	...
	//TOTAL CHARGE!!!	
      //total=(normalcharge[0]/ratio)*200/3;
	  //to have a potential independent of noofranks and size
      //  total=total*dradiusdx[i];
	  printf("before %E total %E dradius %E\n", (normalcharge[0]/ratio), total, dradiusdx[i]);
	  //  getchar();
#ifdef MPI
	  if(numtasks>1)
	    total=total/(numtasks-1);	
#endif    
	  redist=total/dpartlast[i];
      for(ii=0; ii<dpartlast[i]; ii++) 
	{
//	  dpart[i][ii].q=redist;
		dpart[i][ii].q=redist+coeff*(dpart[i][ii].x-dmass_centr_x[i]);
