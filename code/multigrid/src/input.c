...
void convert(void)
{
    ...
    write=my_file_open("newfile.dat", "w");
    do
    {
        l=getc(open);
        if(((l>='0') && (l <='9')) || l=='\n' || l=='E' || l=='.' || l=='-' || l==' ')
...
void readdata(int arc, char *arv[]) 
{
    ...
    if(rank==0)
    {
        fprintf(write,"Grid spacing dx:%E m\nGrid spacing dy:%f m\nGrid spacing dz:%E\n", dx, dy, dz);
...
//        printf("%d\n",diagint_st[i]);
    ...
    /*BEAM BEAM*/
    double factor=4; //here is the beam only in the 25% of the volume we are entering with npart[2] thus npartb/(0.25*V)=dens[2]
    //  factor=1; //whole volume
    dens[2]=npart[2]*1.0*ratio*factor/(Lx*Ly*Lz-dustvolume);
    npart[3]=npart[2];
    dens[3]=dens[2];
    ...
    if(numtasks>1)
    {
        ...
        npartinit[3]=(int)((npart[3]*1.0)/(numtasks-1));
    ...
    charge[3]=charge[0]; //switch for real charge value
    mass[3]=mass[0];
    qm[3]=qm[0];
    ...
    vthx[2]=vthy[2]=vthz[2]=vthx[1]; //isothermal cold beam
    vthx[3]=vthy[3]=vthz[3]=vthx[0];
    vdriftx[3]=0;
    tempx[2]=tempx[1];
    tempy[2]=tempy[1];
    tempz[2]=tempz[1];
