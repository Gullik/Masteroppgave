...
void memorygrid(void)
{
    ...
    /*allocate memorty for MAGNETIC FIELD vectors with grid data*/
    BfMAX=npoints*3*NOF; /*3 stands for 3D vector components of magnetic field*/
    FsBy=npoints; /*Ey offset*/
    FsBz=2*npoints; /*Ez offset*/
    Bf=dvecmem(0,BfMAX-1); //forces...
...
void startBfield()
{
    ...
    for(i=0; i<BfMAX; i++)
        Bf[i]=0.0;
    //30 Gauss in x direction
    for(i=0; i<FsBy; i++)
        //  Bf[i]=0.03*fabs(qm[0])*(1/omegap[0]);
        Bf[i]=0.005*fabs(qm[0])*(1/omegap[0]);
}
...
void weighting1(void)
{
    ...
    for(i=0;i<S;i++)
    {
//        printf("chargeandnorm = %f, dV = %f \n", chargeandnorm[i], dV);
        q=chargeandnorm[i]/(dV);
        printf("specie chargeandnorm %E \n", q);
        for(ii=0;ii<npart[i];ii++)
        {
            ...
            //  printf("specie %d part %d corrd %d %d %d\n", i,ii,j,k,l);
        ...
        printf("before, specie %d\n", i);
        /*for(ij=0; ij<ngy; ij++)
        {
            for(il=0; il<ngz; il++)
            {
                printf("%E\t", rho[ix(0,5,ij,il)]);
            }
        printf("\n");
       }*/
...
#ifdef PERIODIC
    printf("periodic weighting, test it\n \n !!!!!!!!!Double checking it still works.");
    //modified 03.09.2013 WJM corrected factors
    //sides (interior)
    for(ij=1; ij<ngx-1; ij++)
        for(ik=1; ik<ngy-1; ik++)
        {
            rho[ix(0,ij,ik,0)]=1.0*(rho[ix(0,ij,ik,0)]+rho[ix(0,ij,ik,ngz-1)]);
            rho[ix(0,ij,ik,ngz-1)]=rho[ix(0,ij,ik,0)];
        }
    for(ik=1; ik<ngy-1; ik++)
        for(il=1; il<ngz-1; il++)
        {
            rho[ix(0,0,ik,il)]=1.0*(rho[ix(0,0,ik,il)]+rho[ix(0,ngx-1,ik,il)]);
            rho[ix(0,ngx-1,ik,il)]=rho[ix(0,0,ik,il)];
        }
    for(ij=1; ij<ngx-1; ij++)
        for(il=1; il<ngz-1; il++)
        {
            rho[ix(0,ij,0,il)]=1.0*(rho[ix(0,ij,0,il)]+rho[ix(0,ij,ngy-1,il)]);
            rho[ix(0,ij,ngy-1,il)]=rho[ix(0,ij,0,il)];
        }

    //edges
    for(ij=1; ij<ngx-1; ij++)
    {
        rho[ix(0,ij,0,0)]=1.0*(rho[ix(0,ij,0,0)]+rho[ix(0,ij,ngy-1,ngz-1)]+rho[ix(0,ij,0,ngz-1)]+rho[ix(0,ij,ngy-1,0)]);
        rho[ix(0,ij,0,ngz-1)]=rho[ix(0,ij,ngy-1,0)]=rho[ix(0,ij,ngy-1,ngz-1)]=rho[ix(0,ij,0,0)];
    }
    for(ik=1; ik<ngy-1; ik++)
    {
        rho[ix(0,0,ik,0)]=1.0*(rho[ix(0,0,ik,0)]+rho[ix(0,ngy-1,ik,ngz-1)]+rho[ix(0,0,ik,ngz-1)]+rho[ix(0,ngx-1,ik,0)]);
        rho[ix(0,0,ik,ngz-1)]=rho[ix(0,ngx-1,ik,0)]=rho[ix(0,ngx-1,ik,ngz-1)]=rho[ix(0,0,ik,0)];
    }
    for(il=1; il<ngz-1; il++)
    {
        rho[ix(0,0,0,il)]=1.0*(rho[ix(0,0,0,il)]+rho[ix(0,ngx-1,ngy-1,il)]+rho[ix(0,ngx-1,0,il)]+rho[ix(0,0,ngy-1,il)]);
        rho[ix(0,ngx-1,0,il)]=rho[ix(0,0,ngy-1,il)]=rho[ix(0,ngx-1,ngy-1,il)]=rho[ix(0,0,0,il)];
    }

    //corners
    rho[ix(0,0,0,0)]=1.0*(rho[ix(0,ngx-1,ngy-1,ngz-1)]+rho[ix(0,0,ngy-1,ngz-1)]+rho[ix(0,ngx-1,0,ngz-1)]+rho[ix(0,ngx-1,ngy-1,0)]
            +rho[ix(0,0,0,ngz-1)]+rho[ix(0,0,ngy-1,0)]+rho[ix(0,ngx-1,0,0)]+rho[ix(0,0,0,0)]);
    rho[ix(0,ngx-1,ngy-1,ngz-1)]=rho[ix(0,0,ngy-1,ngz-1)]=rho[ix(0,ngx-1,0,ngz-1)]=rho[ix(0,ngx-1,ngy-1,0)]=rho[ix(0,0,0,ngz-1)]=rho[ix(0,0,ngy-1,0)]=rho[ix(0,ngx-1,0,0)]=rho[ix(0,0,0,0)];

#endif	

    printf("AFTER specie %d\n", i);
    /*	for(ij=0; ij<ngy; ij++)
    {
        for(il=0; il<ngz; il++)
        {
